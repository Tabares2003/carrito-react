...
const Cart = () => {
  ...
  return (<div className={styles.cartContainer}>    
      <div 
      onClick={() => {
        setCartOpen(!cartOpen); // Cuando el botón es clickeado, cambiamos el estado de cartOpen a su opuesto
        }} 
        className={styles.buttonCartContainer}>
        <div className={styles.buttonCart}>
            {!cartOpen ? (
              <svg
              className={styles.open} // Si cartOpen es falso, renderizamos el ícono "abierto"
              width={"35px"}
              viewBox="0 0 30 27"
              fill="none"
            >
        ...
        {!cartOpen &&  <div className={styles.productsNumber}>{productsLength}</div>} {/* Si cartOpen es verdadero, renderizamos el menú desplegable del carrito */}
      </div>
...
export default Cart // Exportamos el componente